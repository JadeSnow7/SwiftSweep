...
public class UninstallCacheStore {
  ...
  public func loadCachedApps() -> [CachedAppInfo] {
    ...
    do {
      ...
      let apps = try decoder.decode([CachedAppInfo].self, from: data)
  ...
  public func loadResiduals(for appPath: String) -> [CachedResidualInfo] {
    ...
    do {
      ...
      let decoder = JSONDecoder()
      decoder.dateDecodingStrategy = .iso8601
      let allResiduals = try decoder.decode([CachedResidualInfo].self, from: data)
    ...
    queue.async { [self] in
      do {
        ...
        if FileManager.default.fileExists(atPath: residualsCacheURL.path) {
          ...
          let decoder = JSONDecoder()
          decoder.dateDecodingStrategy = .iso8601
          allResiduals = try decoder.decode([CachedResidualInfo].self, from: data)
