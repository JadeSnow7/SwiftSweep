...
public struct DeveloperCacheRule: RecommendationRule {
  ...
  public func evaluate(context: RecommendationContext) async throws -> [Recommendation] {
    ...
    let overallSeverity: Severity =
      foundCaches.contains { $0.severity == .warning } ? .warning : .info
    ...
    let actions: [Action] = [
      Action(
        ...
        supportsDryRun: true),
      Action(
        type: .openFinder, payload: .paths([home.appendingPathComponent("Library/Developer").path]),
    ...
    recommendations.append(
      Recommendation(
        ...
        summary:
          "\(foundCaches.count) developer caches totaling \(formatBytes(totalSize)). Safe to clean.",
