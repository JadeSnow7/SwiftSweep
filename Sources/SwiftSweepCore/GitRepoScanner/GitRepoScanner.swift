...
public actor GitRepoScanner {
  ...
  /// Get sizes for multiple repos with concurrency limit
  public func getSizes(for repos: [GitRepo]) async -> [String: Int64] {
    ...
    await withTaskGroup(of: (String, Int64?).self) { group in
      ...
      while index < repos.count || pending > 0 {
        while pending < statusConcurrencyLimit && index < repos.count {
          let repo = repos[index]
          group.addTask {
            let size = self.calculateDirectorySize(at: repo.gitDir)
            return (repo.id, size)
          }
          pending += 1
          index += 1
        ...
        if let (id, size) = await group.next() {
          if let s = size {
            results[id] = s
          }
          pending -= 1
  ...
  /// Calculate directory size using FileManager (no external commands)
  /// nonisolated because FileManager is thread-safe
  private nonisolated func calculateDirectorySize(at path: String) -> Int64? {
    ...
    guard
      let enumerator = fm.enumerator(
        ...
        options: []  // Don't skip hidden files - git objects are hidden
      )
    else {
      return nil
    }
