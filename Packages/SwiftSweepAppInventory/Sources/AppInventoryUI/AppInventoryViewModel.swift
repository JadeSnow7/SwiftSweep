...
public enum InventoryState: Equatable {
  ...
  public static func == (lhs: InventoryState, rhs: InventoryState) -> Bool {
    ...
    case (.idle, .idle),
      ...
      return true
    case (.deepScanning(let c1, let t1), .deepScanning(let c2, let t2)):
      return c1 == c2 && t1 == t2
...
public final class AppInventoryViewModel: ObservableObject {
    ...
    do {
      ...
      if apps.isEmpty {
        ...
        apps = contents.compactMap { url -> AppItem? in
          ...
          let contentModified = try? url.resourceValues(forKeys: [.contentModificationDateKey])
  ...
  public var filteredApps: [AppItem] {
    ...
    // Default sort by contentModifiedDate (newest first)
    return result.sorted { lhs, rhs in
      let lhsDate = lhs.contentModifiedDate ?? .distantPast
      let rhsDate = rhs.contentModifiedDate ?? .distantPast
